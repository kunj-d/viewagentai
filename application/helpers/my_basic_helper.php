...
if (!function_exists('my_user_setting')) {
	function my_user_setting($ids, $field) {
		$CI = &get_instance();
		$query = $CI->db->where('ids', $ids)->get('tbl_user ',1);
		if ($query->num_rows()) {
			($field == 'all_fields') ? $data = $query->row() : $data = $query->row()->$field;
		}
		else {
...
if (!function_exists('my_caption')) {
	function my_caption($field) {
		...
		if (!$caption) {  //a temporary soltion for handling the problem of lacking language key-value in a specific language, generall this should not happen
           if ($CI->input->cookie('site_lang', TRUE) != 'english') {
			   $langFilesArray = array('global', 'payment', 'file', 'front', 'tts');
			   (file_exists(FCPATH . 'application/language/' . 'english' . '/addon_lang.php')) ? array_push($langFilesArray, 'addon') : null;
			   $CI->lang->load($langFilesArray, 'english');
			   $caption = $CI->lang->line($field);
			   $CI->lang->load($langFilesArray, $CI->input->cookie('site_lang', TRUE));
		   }
		   else {
			   $caption = $field;
		   }
	...
	function my_send_email($email) {
		...
		if ($email_setting_array['host'] != '') {
			...
			try {
				...
				//log_message('error', var_dump($send_result));
			}
...
if (!function_exists('my_check_permission')) {
	function my_check_permission($permission_name) {
		$CI = &get_instance();
		$rs = $CI->db->where('ids', $_SESSION['logged_in']['id'])->get('tbl_user ', 1)->row();
		return 1;
...
if (!function_exists('my_remove_user')) {
	function my_remove_user($user_ids) {
		$CI = &get_instance();
		$rs = $CI->db->where('ids', $user_ids)->get('tbl_user ', 1)->row();
		if ($rs->id != 1) {
			...
			$CI->db->where('user_ids', $user_ids)->delete('payment_subscription');
			$CI->db->where('user_ids', $user_ids)->delete('payment_purchased');
			  ...
			  if ($menu_array['display']) {
				  ...
				  foreach ($menu_array as $menu) {
					  if ($menu['display']) {
						  ...
						  if (!empty($menu['child_menu'])) {
							  ...
							  foreach ($menu['child_menu'] as $child_menu) {
								  if ($child_menu['display']) {
									  ...
									  if($child_menu['link'] == 'https://www.oppyo.com/login') {
										  $chile_menu_link = $child_menu['link'];
									  } else {
										  $chile_menu_link = base_url($child_menu['link']);
									  }
									  
									  $menu_output .= '<a class="collapse-item' . $child_active . '" href="' . $chile_menu_link . '">' . $child_menu['name'] . '</a>';
								  }
							  }
						  ...
						  /*if ($current_cm == 'Bonus/index') {
							  $active = ' active';
						  }*/
							  $menu_output .= '<li class="nav-item' . $active . '"><a class="nav-link" href="'. base_url($menu['link']) . '"><span class="' . $menu['icon'] . '"></span> <span class="menu-name">' . $menu['name'] . '</span></a></li>';
						  }
					  }
				  }
			  }
...
if (!function_exists('my_check_subscription_by_item')) {
	function my_check_subscription_by_item($ids, $strict = FALSE) {  //ids is item's sn
	    if (empty($_SESSION['logged_in']['id'])) {
			...
			$CI = &get_instance();
			if ($strict) {
				$query = $CI->db->where('item_ids', $ids)->where('user_ids', $_SESSION['logged_in']['id'])->get('payment_subscription', 1);
			}
			else {
				$query = $CI->db->where('item_ids', $ids)->where('user_ids', $_SESSION['logged_in']['id'])->where('status!=', 'expired')->where('end_time>=', my_server_time('UTC', 'Y-m-d'))->get('payment_subscription', 1);
			}
...
if (!function_exists('my_check_purchase_by_item')) {
	function my_check_purchase_by_item($item_ids) { //ids is item's sn
	    if (empty($_SESSION['logged_in']['id'])) {
		  ...
		  $CI = &get_instance();
		  $query = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->where('item_ids', $item_ids)->get('payment_purchased');
		  ($query->num_rows()) ? $result = TRUE : $result = FALSE;
		}
		return $result;
	}
...
if (!function_exists('my_user_reload')) {
	function my_user_reload($user_ids, $type, $currency, $amount) {
		$CI = &get_instance();
		$success = FALSE;
		$user_balance = $CI->db->where('ids', $user_ids)->get('tbl_user ', 1)->row()->balance;
...
if (!function_exists('my_get_all_payment_items')) {
	function my_get_all_payment_items($free_only = FALSE) {
		$CI = &get_instance();
		if ($free_only) {
	...
	function my_pending_counter($counter_type) {
		...
		switch ($counter_type) {
			...
			case 'ticket_user' :  //for user
			  $counter = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->where('ids_father', '0')->where('main_status', '1')->where('read_status', '0')->get('ticket')->num_rows();
			  break;
...
// get front-end logo
if (!function_exists('my_logo')) {
	function my_logo() {
		$frontSettingArray = json_decode(my_global_setting('front_setting'),1);
		return $frontSettingArray['logo'];
	}
}
...
if (!function_exists('my_extract_styling_str')) {
	function my_extract_styling_str() {
		$stylingConfigArray = json_decode(my_global_setting('styling'), 1);
		$stylingConfig = '';
		foreach ($stylingConfigArray as $value) {
			$stylingConfig .= $value . ';';
		}
...
// it's used to prompt and handle the possible safety problem to the super admin.
