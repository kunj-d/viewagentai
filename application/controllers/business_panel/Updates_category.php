...
class Updates_category extends CI_Controller {
 
	function __construct()
	{ 
	    parent::__construct();				
		$this->load->model($this->config->item('adminFolderName').'/Updates_log_category_Model');
	...
	public function index()
	{
		...
		$config['base_url'] = base_url($this->config->item('adminName').'/update-log-version/?keyword='.$keyword);
		$config['per_page'] = (int)$this->config->item("Reading_records_per_page"); //from site settings
		$config['total_rows'] = $this->Updates_log_category_Model->getAllCount('',$keyword);
		$config['page_query_string'] = TRUE;
	    $currentpage = ($this->input->get('per_page')) ? $this->input->get('per_page') : 0;
        $this->pagination->initialize($config);	
	    $output['paging']=$this->pagination->create_links();
		
		$output['list'] = $this->Updates_log_category_Model->getRecordList($keyword,$config['per_page'],$currentpage);
		
		$output['active'] = $this->Updates_log_category_Model->getAllCount('active',$keyword);
		$output['inactive'] = $this->Updates_log_category_Model->getAllCount('inactive',$keyword);
		$output['total'] = $this->Updates_log_category_Model->getAllCount('',$keyword);
		$this->load->view($this->config->item('adminFolderName').'/header',$output);
		$this->load->view($this->config->item('adminFolderName').'/update_log_category/list');
	...
	public function view()
	{
		...
		$output['detail'] = $this->Updates_log_category_Model->getRecordDetail($id);
		$response['html'] = $this->load->view($this->config->item('adminFolderName').'/update_log_category/view',$output,true);
	...
	public function create()
	{
		...
		if($_POST)
		{
			...
			if($this->form_validation->run())
			 {
				   $this->Updates_log_category_Model->addRecord();
				   $this->session->set_userdata('success_msg','Category Created Successfully');
				   redirect($this->config->item('adminName').'/update-log-version');
			 }
		}
		...
		$this->load->view($this->config->item('adminFolderName').'/header',$output);
		$this->load->view($this->config->item('adminFolderName').'/update_log_category/edit');
		$this->load->view($this->config->item('adminFolderName').'/footer');
	}
	public function edit($id)
	{
		...
		$detail = $this->Updates_log_category_Model->getRecordDetail($id);
		$title = $detail->title;
		$slug = $detail->slug;
		...
		if($_POST)
		{
			...
			if($this->form_validation->run())
			 {
			   $this->Updates_log_category_Model->updateRecord($id);
	...
	function delete($id) 
	{
	   ...
	   $this->Updates_log_category_Model->deleteRecord($id);
	...
	function changeStatus($task,$id) 
	{
	   ...
	   $this->Updates_log_category_Model->set_status($task,$id);
