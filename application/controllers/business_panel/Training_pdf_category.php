...
class Training_pdf_category extends CI_Controller {

	function __construct()
	{ 
	    parent::__construct();				
		$this->load->model($this->config->item('adminFolderName').'/Training_pdf_cat_Model');
	...
	public function index()
	{
		 ...
		 $output['manager_id'] = $manager_id = 40;
		...
		$config['base_url'] = base_url($this->config->item('adminName').'/manage-training-pdf-category/?keyword='.$keyword);
		$config['per_page'] = (int)$this->config->item("Reading_records_per_page"); //from site settings
		$config['total_rows'] = $this->Training_pdf_cat_Model->getAllCount('',$keyword);
		$config['page_query_string'] = TRUE;
	    $currentpage = ($this->input->get('per_page')) ? $this->input->get('per_page') : 0;
        $this->pagination->initialize($config);	
	    $output['paging']=$this->pagination->create_links();
		
		$output['list'] = $this->Training_pdf_cat_Model->getRecordList($keyword,$config['per_page'],$currentpage);
		
		$output['active'] = $this->Training_pdf_cat_Model->getAllCount('active',$keyword);
		$output['inactive'] = $this->Training_pdf_cat_Model->getAllCount('inactive',$keyword);
		$output['total'] = $this->Training_pdf_cat_Model->getAllCount('',$keyword);
		$this->load->view($this->config->item('adminFolderName').'/header',$output);
		$this->load->view($this->config->item('adminFolderName').'/training_pdf_cat/list');
	...
	public function view()
	{
		...
		$output['detail'] = $this->Training_pdf_cat_Model->getRecordDetail($id);
		$response['html'] = $this->load->view($this->config->item('adminFolderName').'/training_pdf_cat/view',$output,true);
	...
	public function create()
	{
		...
		if($_POST)
		{
			...
			if($this->form_validation->run())
			 {
				   $this->Training_pdf_cat_Model->addRecord();
				   $this->session->set_userdata('success_msg','Category Created Successfully');
				   redirect($this->config->item('adminName').'/manage-training-pdf-category');
			 }
		}
		...
		$this->load->view($this->config->item('adminFolderName').'/header',$output);
		$this->load->view($this->config->item('adminFolderName').'/training_pdf_cat/edit');
		$this->load->view($this->config->item('adminFolderName').'/footer');
	}
	public function edit($id)
	{
		...
		$detail = $this->Training_pdf_cat_Model->getRecordDetail($id);
		$title = $detail->title;
		$slug = $detail->slug;
		...
		if($_POST)
		{
			...
			if($this->form_validation->run())
			 {
			   $this->Training_pdf_cat_Model->updateRecord($id);
	...
	function delete($id) 
	{
	   ...
	   $this->Training_pdf_cat_Model->deleteRecord($id);
	...
	function changeStatus($task,$id) 
	{
	   ...
	   $this->Training_pdf_cat_Model->set_status($task,$id);
