...
class Conversation_controller extends AppDefault
{
    ...
    {
        // $this->checkassitant($this->business_id);
        if (!empty($_POST['chat_id'])) {
            $data['continue_chat_id'] = $_POST['chat_id'];
        } else {
            $data['last_chatid'] = $this->Conversation_Model->getLastChatID($this->business_id);;
            if (empty($data['last_chatid'])) {
                $this->Conversation_Model->createChat($this->business_id);
                $data['last_chatid'] = $this->Conversation_Model->getLastChatID($this->business_id);
            }
        }
        ...
        $data["assistant_id"] = $this->business_id;
        $data['assistantData'] = $this->Conversation_Model->getAssistantData($this->business_id);
        ...
        // die;
        $data['assistantChatInfo'] = $this->Conversation_Model->getAssistantInfo($this->business_id);
    ...
    public function getchattitle()
    {
        ...
        $data = $this->Conversation_Model->getChatTitle($this->business_id, $this->business_id);
        ...
        //   $va_first_message = 'Hi ' . $user_name . ', I am AiAgentsArmy ( Your Super VA ) How may I help you today?';
        // $va_first_message = 'Hi ' . $user_name .', How can I assist you today?';
            $va_first_message = 'Hi ' . $user_name . ', I\'m your AI Tube Agent. How can I assist you with your YouTube tasks today?';
        }else{
            // $va_first_message = 'Hi ' . $user_name . ', I am AiAgentsArmy ( Your Super VA ) and I have expertise in ' . $assistantData->prompt_cat . ' too. How may I help you today?';
            $va_first_message = 'Hi ' . $user_name . ', I\'m your AI Tube Agent. How can I assist you with your YouTube tasks today?';
            
        }
        $array = [
            'business_id' => $this->business_id,
            'prompt_id' => $this->business_id,
    ...
    public function getchatlist()
    {
        ...
        $chatlist = $this->Conversation_Model->getChatlist($this->business_id, 1);
        $result = array(
            'status' => 1,
            'chatlist' => $chatlist,
            'msg' => 'Success'
 ...
 */
                ...
                $db_user = "tubeengi_tubeeng";  // Your database username
                $db_pass = "_8s3&NYmug$}";  // Your database password
                $db_name = "tubeengi_tube_engine";  // Your database name
    ...
    public function getPromptDetail()
    {
        ...
        if (!$limit) {
            $limit = 16;  // Default to 5 if not provided
        }
        $prompt_detail = $this->db->order_by('id','DESC')->limit($limit, $offset)->get('prompt_detail')->result_array();
        /* 
        pr($prompt_detail);
