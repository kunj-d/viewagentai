...
class Editor extends CI_Controller {
	...
	function __construct() {
		...
		/*if( !$this->session->userdata( 'member_login' ) && !$this->session->userdata( 'admin_member_login' ) ){
		    ...
		    if($this->session->userdata('member_login')){
				...
				$result = $this->Common_Model->get_data('user_subscriptions', $where);
	...
	public function edit( $campaign_id = '', $template_id = '', $sub_user_id = '' ){
		...
		if($template_id !='' && $campaign_id != ''){
			$data = array();
			$userID = $this->user_id;
			$data['images'] = $this->Common_Model->get_data_limit( 'user_image', array( 'user_id' => $userID ), '*', array(0,21), 'id','DESC' );
			
			if(isset($sub_user_id) && !empty($sub_user_id)){
				$sub_users = $this->Common_Model->get_data( 'sub_users', array('parent_user_id'=>$userID,'sub_user_id'=>$sub_user_id,'status'=>1) );
			...
			$data['template'] = $this->Common_Model->get_data( 'user_templates', array( 'user_id' => $userID, 'campaign_id' => $campaign_id, 'template_id' => $template_id, 'status' => 1 ), '*', array() );
		
			$data['theme_setting'] = $this->Common_Model->get_data( 'theme_setting', '', '*', array() );
			
			$data['template_id'] = $template_id;
			$data['campaign_id'] = $campaign_id;
			$data['suggestion_category'] = $this->Common_Model->get_data( 'suggestion_category' );
	
			if(empty($data['template'])) redirect( 'dashboard', 'refresh' );
			$this->load->view( 'default/templates/editor', $data );
		}else{
			redirect( 'dashboard', 'refresh' );
		}
	}
	...
	public function upload_media(){
		if(isset($_FILES['mediafile'])){
            ...
            if (!file_exists($path)) {
                mkdir($path, 0777, true); // Creates the directory recursively
            }
			...
			if ( ! $this->upload->do_upload('mediafile')){
				...
				$insert_id = $this->Common_Model->put_data( 'user_image', $array );
				
				$result['thumb_url'] = base_url(). 'app/uploads/user_'.$userID.'/images/' .$thumb_name . '_thumb.' . $ext;
				$result['image_url'] = base_url(). 'app/uploads/user_'.$userID.'/images/' . $name;
				$result['success'] = 1;
				$result['id'] = $insert_id;
	...
	public function deleteimage(){
		if(!empty($_POST['id']) && $_POST['delete'] == 'confirm'){
			...
			$image = $this->Common_Model->get_data( 'user_image', $where );
			$userID = $this->user_id;
			if(!empty($image)){
				...
				if(unlink($full_path) && unlink($thumb_path)){
					$this->Common_Model->delete_data( 'user_image', $where );
	...
	public function save_template(){
		if(isset($_POST['template_data']) && $_POST['template_id'] == 0){
			...
			$redirectUrl = base_url('my_templates');
			...
			$image = $this->Common_Model->get_data( 'user_templates', $where );
			$size = '';
			if(!count($image)){
			...
			$userID = $this->user_id;
			$directory = './uploads/user_'.$userID.'/templates/';
        ...
        fclose($myfile);
			...
			echo json_encode( array( 'status' => 1, 'msg' =>html_escape($this->lang->line('ltr_editor_save_template_msg2')), 'thumb' => 'uploads/user_'.$userID.'/templates/' . $name, 'size' => $size, 'access_level' => $this->session->userdata( 'access_level' ), 'redirectUrl' => $redirectUrl ) );
		}
...
// 		}
	...
	public function get_object(){
		...
		if(isset($_POST['template_id'])){
			...
			/*if(isset($_POST['sub_user_id']) && !empty($_POST['sub_user_id'])){
				$sub_users = $this->Common_Model->get_data( 'sub_users', array('parent_user_id'=>$userID,'sub_user_id'=>$_POST['sub_user_id'],'status'=>1) );
				if(count($sub_users) == 1){
			...
			$template_data = $this->Common_Model->get_data( 'user_templates', $where, 'template_data,gradient_background,sub_cat_id,template_size,template_custom_size', array() );
			...
			if(isset($template_data[0])){
				...
				if(!$this->session->userdata( 'admin_member_login' )){
					...
					$suggestion = $this->Common_Model->get_data( 'suggestion', $w );
					$tips = $this->Common_Model->get_data( 'suggestion', $w_suggestion, '*' );
				}
	...
	public function admin_edit( $template_id = '' ){
		...
		$userID = $this->user_id;
		$data['images'] = $this->Common_Model->get_data( 'user_image', array( 'user_id' => $userID ), '*', array( 'id'=>'DESC' ) );
		$data['userID'] = $userID;
		$data['template'] = $this->Common_Model->get_data( 'user_templates', array( 'template_id' => $template_id ), '*', array() );
		$data['template_id'] = $template_id;
		$data['theme_setting'] = $this->Common_Model->get_data( 'theme_setting', '', '*', array() );
...
//         die;
	...
	public function more_images(){
		if(isset($_POST['page'])){
			...
			$s = 21 * $page;
			$e = $s + 21;
			$images = $this->Common_Model->get_data_limit( 'user_image', array( 'user_id' => $userID ), '*', array($s,$e), 'id','DESC' );
			if(count($images)){
