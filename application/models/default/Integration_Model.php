...
class Integration_Model extends CI_model
{
	...
	public function get_autoresponder_list(){	
		...
		$this->db->select("users_autoresponder_settings.*,autoresponders.title,autoresponders.id");
		$this->db->from("users_autoresponder_settings");
		$this->db->join('autoresponders','autoresponders.id=users_autoresponder_settings.autoresponder_id','LEFT');

		$this->db->where_in('users_autoresponder_settings.user_id',$this->session->userdata('logged_in')['id']);
		$this->db->where_in('users_autoresponder_settings.business_id',$this->session->userdata('business')['id']);
		//$this->db->group_by('templates.id');
	//	$this->db->order_by('count_data', 'desc'); 
		$query=$this->db->get();
	...
	public function get_autoresponder_list1(){	

		$this->db->select("users_autoresponder_settings.*,autoresponders.title,autoresponders.id");
		$this->db->from("users_autoresponder_settings");
		$this->db->join('autoresponders','autoresponders.id=users_autoresponder_settings.autoresponder_id','LEFT');
		$this->db->where('users_autoresponder_settings.autoresponder_id !=','39');
	...
	public function get_autoresponder_listIdTitle(){	

		$this->db->select("autoresponders.id,autoresponders.title");
		$this->db->from("users_autoresponder_settings");
		$this->db->join('autoresponders','autoresponders.id=users_autoresponder_settings.autoresponder_id','LEFT');
		$this->db->where('users_autoresponder_settings.autoresponder_id !=','39');
	...
	public function getAutoresponderProfile(){
		...
		$autoresponder_list['contentprovider']=array();
		$autoresponder_list['pinecone']=array();
		$autoresponder_list['payment']=array();
		$autoresponder_list['workflow']=array();
		
		if(!empty($_POST['search'])){
		    $this->db->like('display_title',$_POST['search'],'both');
		    $this->db->or_like('title',$_POST['search'],'both');
		}
	...
	public function getWebinarList($autoresponder_id,$user_id){
		...
		if(($responder_info->modified+3600)<time()){
			...
			try {
				$oauth = $citrix->refreshToken($credentials->refresh_token,$credentials->api_key,$credentials->consumer_secret,base_url('integration-settings'));
