...
class M_billing {
	...
	public function billing($config) {
		...
		if (my_check_permission('TTS Management')) {  // TTS admin is alway free to use
			...
			//echo"<pre>";print_r($_SESSION['logged_in']['id']); die('12');
			// checking steps:
			// 1. Is it free?
	...
	protected function payg_billing($config) {
		global $CI;
		$result = FALSE;
		$query_statitics = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->get('tts_statitics', 1);
	...
	protected function deduct_payg_balance($config) {
		global $CI;
		$rs_statitics = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->get('tts_statitics', 1)->row();
		...
		if ($balance_in_statitics >= 0) {  //remain amount of payg package is enough
		    if ($config['synthesize_type'] == 'save') {
				$CI->db->where('user_ids', $_SESSION['logged_in']['id'])->update('tts_statitics', array('payg_balance'=>$balance_in_statitics));
			}
			$result = TRUE;
		}
		else {
	...
	protected function deduct_package_balance($config, $package_ids) {
		...
		if (!$package_type) {
			...
			if ($package_type == 'purchase') {  //type of purchase
				...
				$query = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->where('used_up', 0)->where('item_ids', $package_ids)->order_by('id', 'asc')->get('payment_purchased');
				if ($query->num_rows() > 1) {  //more than one valid and same package, combine them
					$rs = $query->result();
					$cnt = 1;
					$availableCharactersSum = 0;
					foreach ($rs as $row) {
						if ($cnt > 1) {
							$stuff_array = json_decode($row->stuff, TRUE);
							$availableCharactersSum += $stuff_array['characters_limit'] - $stuff_array['characters_used'];
							$stuff_array['characters_used'] = $stuff_array['characters_limit'];
							//$CI->db->where('ids', $row->ids)->update('payment_purchased', array('stuff'=>json_encode($stuff_array), 'used_up'=>1));
						}
						else {
							$baseLinePkgIds = $row->ids;
							$baseLinePkgStuff = $row->stuff;
						}
						$cnt++;
					}
					$stuff_array = json_decode($baseLinePkgStuff, 1);
					$stuff_array['characters_limit'] += $availableCharactersSum;
				//	$CI->db->where('ids', $baseLinePkgIds)->update('payment_purchased', array('stuff'=>json_encode($stuff_array)));
				}
				//$query = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->where('used_up', 0)->where('item_ids', $package_ids)->get('payment_purchased', 1);
			}
			elseif ($package_type == 'subscription') {  //type of subscription
				$tbl = 'payment_subscription';
				$query = $CI->db->where('user_ids', $_SESSION['logged_in']['id'])->where('used_up', 0)->where('item_ids', $package_ids)->where('status!=', 'expired')->get('payment_subscription', 1);
			}
			if ($query->num_rows()) {
				$rs = $query->row();
				...
				if ($stuff_array['characters_limit'] != '' && $config['synthesize_type'] == 'save') {  //has limit for the package, need to deduct
					($config['engine'] == 'standard') ? $characters_used = $stuff_array['characters_used'] + $config['characters_count'] : $characters_used = $stuff_array['characters_used'] + 4*$config['characters_count'];
					if ($characters_used > $stuff_array['characters_limit']) {
						$result = FALSE;
					}
					else {
	...
	protected function buy_payg_package($config) {
		...
		$buy_result_json = my_user_reload($_SESSION['logged_in']['id'], 'Cut', $currency, $price);
