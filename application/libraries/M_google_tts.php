...
class M_google_tts {
	...
	public function init() {
		...
		$google_config = json_decode($rsTtsConfiguration->google, TRUE);
		try {
	...
	public function syncResource() {   //get voice list from google, then insert into table 'tts_resource'
		...
		if ($client) {
			try {
				...
				$rsTtsConfiguration = $CI->db->get('tts_configuration', 1)->row();
				$tts_config_array = json_decode($rsTtsConfiguration->pricing_model, TRUE);
				foreach ($voices as $voice) {
					...
					(substr($voice_id, 0, 6) == 'nb-no-') ? $voice_id = str_replace('nb-no-', 'nb-NO-', $voice_id) : null; //correct an inconsistent wording of Google
					(substr($voice_id, 0, 6) == 'bg-bg-') ? $voice_id = str_replace('bg-bg-', 'bg-BG-', $voice_id) : null; //correct an inconsistent wording of Google
					(substr($voice_id, 0, 6) == 'ca-es-') ? $voice_id = str_replace('ca-es-', 'ca-ES-', $voice_id) : null; //correct an inconsistent wording of Google
					(substr($voice_id, 0, 6) == 'is-is-') ? $voice_id = str_replace('is-is-', 'is-IS-', $voice_id) : null; //correct an inconsistent wording of Google
					(substr($voice_id, 0, 6) == 'lv-lv-') ? $voice_id = str_replace('lv-lv-', 'lv-LV-', $voice_id) : null; //correct an inconsistent wording of Google
