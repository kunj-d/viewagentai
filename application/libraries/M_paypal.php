...
class M_paypal {
	...
	public function newPlan($planArray) {
		...
		if ($accessTokenArray['success']) {
			$action = 'newPlan';
			if ($planArray['currency'] == 'VND') {  //this is a temporary solution for Vietnam user only.
				$planArray['currency'] = 'USD';
				$planArray['price'] = number_format($planArray['price']/22750,2);
			}
			$url = $this->getRestURL($action);
			$header = $this->headerBuilder($action, $accessTokenArray['access_token']);
