...
class M_aws_s3 {
	...
	public function putObject($config, $objectType = 'tts') {
		global $CI;
		...
		if ($client) {
			try {
				if ($objectType == 'tts') {
					...
					($aws_config_array['folder'] != '') ? $full_key = $aws_config_array['folder'] . '/stt_' . $config['ids'] . '.' . $config['output_format'] : $full_key = 'stt_' . $config['ids'] . '.' . $config['output_format'];
				}
