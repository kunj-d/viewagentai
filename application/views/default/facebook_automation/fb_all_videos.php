	...
	<div class="container-fluid container-padding padding-bonus" id="libCtrl">
		<div class="col-12" ng-if="all_videos.length === 0 && schedule_videos.length === 0" ng-cloak>
			<div class="create-first">
				...
				<p class="description">
					You Haven’t Uploaded Any Video on Facebook Yet
				</p>
				<a href="<?php echo base_url('facebook-video'); ?>" class="btn btn-primary">Upload New Video</a>
			</div>
		</div>
				...
				<div class="feature-banner">
					<div class="row align-items-center justify-content-between g-0">
						<div class="col-auto feature-wrap">
							<h5 class="feature-title">Create and Manage Facebook Posts</h5>
							<p class="feature-subtitle mb-0">
								Schedule, publish, and track post performance in one place.
							</p>
						</div>
						<div class="col-auto ms-auto">
							<a href="<?php echo base_url('facebook-video'); ?>" class=" btn btn-primary"><i class="fa-solid fa-plus"></i> Create New Post </a>
						</div>
					</div>
				</div>
			</div>
...
<!--						<div class="avatar-wrapper-inner">-->
	...
	<script>
		...
		app.controller('MyCtrl', function($scope, $http, $timeout, $parse, $sce) {
			...
			$scope.get_facebook_videos = function(afterCursor = "") {

				let url = "<?php echo base_url('get-fb-insights') ?>";
			...
			// First call
			$scope.get_facebook_videos();

			$scope.loadMore = function() {
				if ($scope.next_cursor) {
					$scope.get_facebook_videos($scope.next_cursor);
				}
			};
			...
			$scope.getschedulevideoList = function() {
				var queryStr = "<?php echo base_url('get-fb-schedule-video') ?>";
				$http({
					method: 'POST',
					url: queryStr,
					headers: {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                   ...
                   $http({
		                method: 'POST',
		                url: "<?php echo base_url('delete-fb-schedule-video') ?>",
		                data: $.param({
                                    video_id: videoid
                                }),
		                headers: {
        ...
        $scope.getschedulevideoList();
			...
			$scope.uploadinstagram = function(video_url) {
				...
				form.method = "POST";
				form.action = "<?php echo base_url('facebook-video'); ?>";
