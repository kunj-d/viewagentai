  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3> Templates Manager <small> Saglus Template Manager </small> </h3>
		<br/>
      </div>
            ...
            <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
			  ...
			  <?php 	foreach($list as $val) { ?>
                ...
                <tr>
                  ...
                  <td><?=$val->title?></td>                
                  <td><img src="<?php echo $this->config->item('uploadPath').'templates/thumbnail/template'.$val->id.'.png'?>"height="50px" width="50px"/></td>
				  <td><?=$val->slug?></td>
				    <td><?php 
							foreach($template_type as $temp_type) { 
								if($temp_type->id==$val->type){
									echo $temp_type->type;
								}; 
							}
					?></td>
               <td><?php foreach($package_plans as $plans){
						if($plans->template_id==$val->id)
							{	
								echo ucwords($plans->sell_type)."-".$plans->title." => ".$plans->price;
								if($plans->price!=""){ echo $plan->price," $, <br>" ;} else { echo " Free, <br>"; };
							}
					}?>
				</td>
				  ...
				  <?php if($status_per=='yes') { ?>
				    <?php if($val->status=='inactive'){ ?>
				       <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Active" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-template-manager-status/active/'.$val->id?>"><i class="fa fa-check"></i> </a>
					<?php } else { ?>   
					   <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Inactive" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-template-manager-status/inactive/'.$val->id?>"><i class="fa fa-ban"></i> </a>
					<?php } } ?>
					  ...
					  <?php } ?>
					   ...
					   <?php if($edit_per=='yes') { ?>
                      <a href="<?=$this->config->item('spanel_url').'edit-template-manager/'.$val->id?>" class="btn btn-info btn-xs" title="Edit" data-toggle="tooltip" data-placement="top"><i class="fa fa-pencil"></i> </a>
					  <?php } ?>
					  
					   <?php if($delete_per=='yes') { ?>
                      <a href="javascript:" class="btn btn-danger btn-xs deletelink" title="Delete" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'delete-template-manager/'.$val->id?>"><i class="fa fa-trash-o"></i> </a>
					  <?php } ?>
				  </td>
                </tr>
			<?php } ?>
...
$('body').delegate(".viewlink", 'click', function() {
    ...
    var posturl = spanel_url+'view-template-manager';
