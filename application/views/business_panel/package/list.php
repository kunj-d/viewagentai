  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3> Packages <small> Saglus package manager </small> </h3>
		<br/>
      </div>
            ...
            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
			  ...
			  <?php foreach($list as $val) { ?>
                <tr>
                  ...
                  <td><?=$val->sell_type?></td>
				  <td><?=$val->plan_name?></td>
				  <td><?=($val->jvz_product_id?$val->jvz_product_id:'N/A')?></td>
                  <td><button type="button" class="btn btn-success btn-xs"><?=$val->status?></button></td>
                  <td>
				  <?php if($status_per=='yes') { ?>
				    <?php if($val->status=='inactive'){ ?>
				       <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Active" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-package-status/active/'.$val->id?>"><i class="fa fa-check"></i> </a>
					<?php } else { ?>   
					   <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Inactive" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-package-status/inactive/'.$val->id?>"><i class="fa fa-ban"></i> </a>
					<?php } } ?>
					  ...
					  <?php } ?>
					   ...
					   <?php /* if($edit_per=='yes') { ?>
                      <a href="<?=$this->config->item('spanel_url').'edit-package/'.$val->id?>" class="btn btn-info btn-xs" title="Edit" data-toggle="tooltip" data-placement="top"><i class="fa fa-pencil"></i> </a>
					  <?php } */?>
					  
					   <?php if($delete_per=='yes') { ?>
                      <a href="javascript:" class="btn btn-danger btn-xs deletelink" title="Delete" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'delete-package/'.$val->id?>"><i class="fa fa-trash-o"></i> </a>
					  <?php } ?>
				  </td>
                </tr>
			<?php } ?>
...
$('body').delegate(".deletelink", 'click', function() {
	...
	bootbox.confirm("Are you sure to delete this package?", function(result) {
	  if(result) {
