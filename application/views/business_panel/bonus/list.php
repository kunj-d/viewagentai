  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3> Bonus <small> Saglus Bonus Manager </small> </h3>
		<br/>
      </div>
          ...
          <div class="x_content">
            <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
			  ...
			  <?php foreach($list as $val) { ?>
                <tr>
                  <td><?=$val->title?></td>
				  <td><img src="<?php echo $this->config->item('uploadPath').'bonus_image/'.$val->image;?>" style="height:50px; width:50px"/></td>
				  <td><?=$val->url?></td>
				  ...
				  <?php if($status_per=='yes') { ?>
				    <?php if($val->status=='inactive'){ ?>
				       <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Active" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-bonus-status/active/'.$val->id?>"><i class="fa fa-check"></i> </a>
					<?php } else { ?>   
					   <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Inactive" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-bonus-status/inactive/'.$val->id?>"><i class="fa fa-ban"></i> </a>
					<?php } } ?>
					  ...
					  <?php } ?>
					   ...
					   <?php if($edit_per=='yes') { ?>
                      <a href="<?=$this->config->item('spanel_url').'edit-bonus/'.$val->id?>" class="btn btn-info btn-xs" title="Edit" data-toggle="tooltip" data-placement="top"><i class="fa fa-pencil"></i> </a>
					  <?php } ?>
					  
					   <?php if($delete_per=='yes') { ?>
                      <a href="javascript:" class="btn btn-danger btn-xs deletelink" title="Delete" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'delete-bonus/'.$val->id?>"><i class="fa fa-trash-o"></i> </a>
					  <?php } ?>
				  </td>
                </tr>
			<?php } ?>
...
$('body').delegate(".viewlink", 'click', function() {
    ...
    var posturl = spanel_url+'view-bonus';
...
$('body').delegate(".deletelink", 'click', function() {
	...
	bootbox.confirm("Are you sure to delete this Bonus?", function(result) {
	  if(result) {
...
$('body').delegate(".statuslink", 'click', function() {
	...
	bootbox.confirm("Are you sure to Change status of this Bonus?", function(result) {
	  if(result) {
