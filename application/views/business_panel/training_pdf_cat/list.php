  ...
  <div class="">
    <div class="page-title">
      <div class="col-md-8 col-sm-7 col-xs-12">
        <h3> Training Pdf Categories <small> Saglus Training Pdf Categories Manager </small> </h3>
		<br/>
      </div>
        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
                    <li><a href="<?=$this->config->item('spanel_url').'create-training-pdf-category'?>" class="btn-success"><i class="fa fa-plus"></i> Add &nbsp;</a>
                    </li>
                  </ul>
			<?php } ?>
            <div class="clearfix"></div>
                ...
                <tr>
				  ...
				  <?php if($status_per=='yes') { ?>
				    <?php if($val->status=='inactive'){ ?>
				       <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Active" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-training-pdf-category-status/active/'.$val->id?>"><i class="fa fa-check"></i> </a>
					<?php } else { ?>   
					   <a href="javascript:" class="btn btn-warning btn-xs statuslink" title="Set Inactive" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'change-training-pdf-category-status/inactive/'.$val->id?>"><i class="fa fa-ban"></i> </a>
					<?php } } ?>
					  ...
					  <?php } ?>
					   ...
					   <?php if($edit_per=='yes') { ?>
                      <a href="<?=$this->config->item('spanel_url').'edit-training-pdf-category/'.$val->id?>" class="btn btn-info btn-xs" title="Edit" data-toggle="tooltip" data-placement="top"><i class="fa fa-pencil"></i> </a>
					  <?php } ?>
					  
					   <?php if($delete_per=='yes') { ?>
                      <a href="javascript:" class="btn btn-danger btn-xs deletelink" title="Delete" data-toggle="tooltip" data-placement="top" data-href="<?=$this->config->item('spanel_url').'delete-training-pdf-category/'.$val->id?>"><i class="fa fa-trash-o"></i> </a>
					  <?php } ?>
				  </td>
                </tr>
			<?php } ?>
...
$('body').delegate(".viewlink", 'click', function() {
    ...
    var posturl = spanel_url+'view-training-pdf-category';
