...
"use strict";
	...
	$('.ed_campaign_select_chng').on('change',function(e){
		e.preventDefault();
		if($(this).val() == ''){
			$('.campaign_name_label').show();
		}else{
			$('.campaign_name_label').hide();
		}
	});
	...
	$('#redirect_bundle').on('change',function(e){
		e.preventDefault();
		if($(this).val() != 'custom'){
			var url = ajaxurl + 'templates?size=' + $(this).val();
			window.location = url;
		}else{
			/*var html = '';
			...
			html += '<div class="col-md-12 mt-2">';
				html += '<label for="img_width">Image Width</label>';
				html += '<input type="text" value="" class="form-control" id="img_width">';
			html += '</div>';
			html += '<div class="col-md-12 mt-2">';
				html += '<label for="img_height">Image Height</label>';
				html += '<input type="text" value="" class="form-control" id="img_height">';
			html += '</div>';
...
// 	html += '<div class="col-md-4 mt20">';
// 		html += '<div class="appoint-wall">';
// 			html += '<div class="row align-items-center">';
// 				html += '<div class="col-12">';
// 					html += '<img src="'+(template.thumb !=  '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'assets/images/'+(template.template_size == '628x628' ? 'empty_campaign.jpg' : 'empty_campaign_long.jpg'))+'" alt="" class="w-100">';
// 				html += '</div>';
// 				html += '<div class="col-12 text-center">';
// 					html += '<h5 class="m-3">'+template_name+'</h5>';

// 					if(!use){
// 						html += '<a href="" class="pg-template-status" title="'+template_name+'">';
// 							html += '<span class="pg-template-status-center">';
// 								html += '<span class="btn btn-primary ed_open_image" data-mfp-src="'+(template.thumb !=  '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'assets/images/'+(template.template_size == '628x628' ? 'empty_campaign.jpg' : 'empty_campaign_long.jpg'))+'">View Template</span>';
// 								html += '<span data-mfp-src="#user-prebuild-existing-template" class="btn btn-primary pg-popup-link ms-1" data-template_userID="'+template.user_id+'" data-get_template_id="'+template.template_id+'">Use this template</span>';
// 							html += '</span>';
// 						html += '</a>';
// 					}else{
// 						html += '<a href="'+ajaxurl+'campaign/use_template/'+template.user_id+'/'+template.template_id+'" class="pg-template-status">';
// 							html += '<span class="pg-template-status-center">';
// 								html += '<span class="btn btn-primary use_temp_btn">Use this template</span>';
// 							html += '</span>';
// 						html += '</a>';
// 					}
...
// 	/*html += '<div class="col-md-4 mt20 pg-template-box">';
// 		html += '<div class="appoint-wall pg-template-box-inner">';
// 			html += '<div class="row pg-template-thumb">';
// 			html += '<div class="col-12">';
// 				html += '<img src="'+(template.thumb !=  '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'assets/images/'+(template.template_size == '628x628' ? 'empty_campaign.jpg' : 'empty_campaign_long.jpg'))+'" alt="">';
// 					html += '</div>';
// 				if(!use){
// 					html += '<a href="" class="pg-template-status" title="'+template_name+'">';
// 						html += '<span class="pg-template-status-center">';
// 							html += '<span class="pg-btn ed_open_image" data-mfp-src="'+(template.thumb !=  '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'assets/images/'+(template.template_size == '628x628' ? 'empty_campaign.jpg' : 'empty_campaign_long.jpg'))+'">View Template</span>';
...
function add_predefind_template(template, ajaxurl, use){
	...
	html += '<div class="col-md-4 col-xl-3 mt20">';
	html += '<div class="appoint-wall template-editor-wrapper">';
	html += '<div class="media">';
	html += '<img src="'+(template.thumb != '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'app/assets/images/'+(template.template_size == '628x628' ? 'empty_campaign.jpg' : 'empty_campaign_long.jpg'))+'" alt="" class="w-100">';
	html += '<div class="template-buttons">';

	if(!use){
		html += '<div class="flex grid-buttons text-center">';
		html += '<a title="View Template" class="grid-image-view text-center ed_open_image" data-mfp-src="'+(template.thumb != '' ? ajaxurl +'app/'+ template.thumb : ajaxurl + 'app/assets/images/empty_campaign.jpg')+'"><i class="fa-solid fa-eye"></i></a>';
		html += '<a title="Use this template" data-mfp-src="#user-prebuild-existing-template" class="grid-image-view text-center pg-popup-link" data-template_userID="'+template.user_id+'" data-get_template_id="'+template.template_id+'"><i class="fa-solid fa-file-pen"></i></a>';
		html += '</div>';
	}else{
		html += '<div class=" create-btn">';
		html += '<a href="'+ajaxurl+'campaign/use_template/'+template.user_id+'/'+template.template_id+'" class="btn btn-primary d-block w-100">Use this template</a>';
		html += '</div>';
	}

	html += '</div>';
	html += '</div>';
	html += '<div class="template-title">';
	html += '<p class="m-0">'+template_name+'</p>';
	html += '</div>';
	html += '</div>';
	html += '</div>';
