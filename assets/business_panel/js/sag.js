...
function addMore()
{
	...
	$('#sag_container').on("click", ".sag_remove", function(e){ //user click on remove text links
        e.preventDefault();
		var sag_element = $(this).attr('sag_element');
...
 	}); 					
					 ...
					 if(sag_data_type=='text'){
				 
				var parsed = $.parseJSON(response);
				$("#response_message").html(parsed.msg).fadeOut(2000);
			if (typeof parsed.url !== typeof undefined && parsed.url !== false) {	
...
$('.sag_submit').on('submit',function(event){ 
		...
		beforeSend: function()
		 ...
		 $(".error-message").remove();
			...
			success: function(response){
				var parsed = $.parseJSON(response);
				$("#response_message").html(parsed.msg).fadeIn(3000).delay(1000).fadeOut("slow");
			if (typeof parsed.url !== typeof undefined && parsed.url !== false) {	
				window.location.href = parsed.url;
			}else {
			}
