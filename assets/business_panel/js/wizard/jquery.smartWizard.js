...
$(document).ready(function(){
    ...
    $('#wizard').smartWizard({onLeaveStep:leaveAStepCallback, onFinish:onFinishCallback});
      ...
      function validateSteps(stepnumber){
        ...
        if(stepnumber == 1){
            ...
            $('.requiredField1').each(function() {
                checkedval = $(this).parent('[class*="icheckbox_flat-green"]').hasClass("checked");
				if(checkedval) { return false; }
            });
			
			if(checkedval==false) {
				    PNotify.removeAll();
					new PNotify({ title: 'Error', text: 'Choose any app first', type: 'error' });
					isStepValid  = false;
					$('#wizard').smartWizard({onShowStep:false});
                } else { isStepValid  = true;  }
        } else if(stepnumber == 2){
			
			 $('.requiredField2').each(function() {
			 ...
			 if(isStepValid==false){
					PNotify.removeAll();
                   new PNotify({ title: 'Error', text: 'Please check the error', type: 'error' });
					}
			 
			 
		} else if(stepnumber == 3){
			
			  $('.requiredField3').each(function() {
					$(this).removeClass('parsley-error');
					...
					if(name=='price')
					 {
					   var planTpVal = $('input[name="plan_type"]:checked').val();
					    if(planTpVal=='paid' && $.trim($(this).val())=='')
						 {
							$(this).addClass('parsley-error');
							isStepValid = false;
						  }
      ...
      function validateAllSteps(){
		...
		//$('.step_descr').next().trigger('click');
		$('.step_descr').each(function(key) {
			isStepValid = validateSteps(key+1)	
			if(isStepValid==false)
...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        next : $('<a>'+options.labelNext+'</a>').attr("href","#").addClass("btn btn-success next"),
    ...
    var _loadContent = function($this, stepIdx) {
		...
		var app_id = $('input.app_id:checked').map(function()
                      { return $(this).val(); }).get();
        ...
        if (ajaxurl && ajaxurl.length>0) {
            ...
            if ($this.options.contentCache && hasContent && resubmit_app==false && stepNum!=4) {
                _showStep($this, stepIdx);
            } else {
                var ajax_args = {
                    url: ajaxurl,
                    type: $this.options.ajaxType,
                    data: ({step_number : stepNum,app_id:app_id,formdata:formdata}),
                    dataType: "text",
                    beforeSend: function(){
                        $this.loader.show();
                    },
...
// Default Properties and Events
    $.fn.smartWizard.defaults = {
        ...
        contentURL:spanel_url+'get-app-fields', // content url, Enables Ajax content loading
