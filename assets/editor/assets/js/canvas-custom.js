(function( $ ) {
	...
	$(function() {
		...
		$(document).on('click','#save_template',function(e){
			...
			data['ebook_id'] = $('#ebook_id').val();
			var book_cover_url = $('#book_cover_url').val();
			$('.ed_preloader ').show();
		...
		$(document).on('click','#save_book_cover',function(e){
			e.preventDefault();
			var data = {};
			var m = canvas.toDataURL({format: "jpg"});
			data['thumb'] = m;	
			...
			data['template_data'] = JSON.stringify(canvas.toJSON(properties_to_save));
			data['template_id'] = $('#template_id').val();
			data['front_cover_tempate_id'] = $('#front_cover_tempate_id').val();
			data['ebook_id'] = $('#ebook_id').val();
			var book_cover_url = $('#book_cover_url').val();
			$('.ed_preloader ').show();
			$('.pg-preloader-wrap').css('display','flex');
			$.ajax({
				url: ajaxurl + 'editor/save_book_cover', 
				type: "POST",         
				data: data, 
				success: function(data){
					var result = jQuery.parseJSON(data);
...
// 		});
		...
		$(document).on('click','.ed_bg_pattern',function(e){
			...
			//$('.ed_pattern_colorpicker').find('.pg-colorPicker-container .pg_colorPicker').colorpicker('destroy');
		...
		$(document).on('click', '.pg_canvas_add_image', function(e){
			...
			// fabric.Image.fromURL(url,  function(b) {
			// 	b.left = 250;
			// 	b.top = 350;
			// 	b.width = 200;
			// 	b.height =200;
			// 	b.opacity = 1;
			// 	b.orig_src = url;
			// canvas.insertAt(b, canvas.getObjects().indexOf(canvas.getActiveObject()));
			// canvas.add(b);
			// console.log(url)
			// console.log(b)
			// });
		...
		/*
		$('.ed_group_colorPicker').colorpicker({
			customClass: 'colorpicker-2x',
			align: 'left',
			sliders: {
				saturation: {
		...
		if($('#template_id').val() != ''){
			...
			$.ajax({
				...
				success:function(data){
					...
					var result = jQuery.parseJSON(data);
					// console.log(jQuery.parseJSON(result.data).objects);
                    // console.log("xxxxxxxxxxxx",jQuery.parseJSON(result.data));
					if(result.status){
						var size = result.size;
						var size_arr = size.split('x');
						canvas_width = parseInt(size_arr[0], 10);
						canvas_height = parseInt(size_arr[1], 10);
						// console.log(typeof(canvas_width), typeof(canvas_height));
						canvas.setWidth(canvas_width).setHeight(canvas_height);
						set_canvas_size(canvas_width, canvas_height);	 
	...
	function set_object_pos() {
		...
		if (a.originX == "center") {
			jQuery("#ed_object_x_position").val(parseInt(a.left - a.getWidth() / 2));
			jQuery("#ed_object_y_position").val(parseInt(a.top - a.getHeight() / 2))
		} else {
	...
	function bootstrap_colorpicker_js( where = null ){
		...
		if(where == null){
			...
			// 		let d1=obj1.filter(d1=>d1.fill == "#4bae4f")
			// 		if(d1.length>0)
			// 		{
			// 		for(let i=0; i<d1.length;i++)
			// 		{
	...
	const msr = 650;
        ...
        reader.onload = (event) => {
	        ...
	        image.onload = (e) => {
                ...
                var ctx = canvas.getContext('2d');
                if(msr < e.target.width || msr < e.target.height ){
                    if(e.target.width > e.target.height){
                        let aspectRatio = msr/e.target.width;
                        canvas.width = msr;
                        canvas.height = e.target.height * aspectRatio;
                    }else  if(e.target.height > e.target.width){
                        let aspectRatio = msr/e.target.height;
                        canvas.height = msr;
                        canvas.width = e.target.width * aspectRatio;
                    }else if(e.target.height = e.target.width){
                ...
                canvas.toBlob(function(blob) {
                    console.log(blob)
                    form_data.append('mediafile', blob);
	            ...
	            success: function(data) {
	                ...
	                if (result.success) {
	                    if (type == 'bg') {
	                        ...
	                        canvas_add_image(result.image_url, 150, 150);
	                    }
	...
	function canvas_add_image(b, desiredWidth, desiredHeight) {
	    var j = new Image();
	    j.src = b;
	    j.onload = function() {
	        var p = new fabric.Image(j);
	        var aspectRatio = j.width / j.height;
	        var scaleFactor = 1;
	        console.log(j.width);
	        console.log(j.height);
	        if (j.width > desiredWidth || j.height > desiredHeight) {
	            if (aspectRatio > 1) { 
	                scaleFactor = desiredWidth / j.width;
	            } else {
	                scaleFactor = desiredHeight / j.height;
	            }
	        }
