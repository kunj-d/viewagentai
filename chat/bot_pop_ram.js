document.addEventListener("DOMContentLoaded", function () {
    function parseInvalidJson(invalidJson) {
        const validJson = invalidJson.replace(/'/g, '"');
        return JSON.parse(validJson);
    }
    var copilot_bot_data = document.querySelector("script[copilot_bot_data]"); 
    const pop_data = parseInvalidJson(copilot_bot_data);
    
    var element_id_script_pop = pop_data.element_id_pop; 
    if(element_id_script_pop){
         element_id_pop = pop_data.element_id_pop; 
    }
    if(!element_id_pop){
         element_id_pop = "u9sst8htz0xhrrds";
    } 
    const prompt_id_pop = pop_data.id_pop;
    const user_id_pop =  pop_data.user_id_pop;
    const business_id_pop = pop_data.business_id_pop;
    const src_pop = document.querySelector("script[src]");
