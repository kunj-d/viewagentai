document.addEventListener('DOMContentLoaded', function() {
    ...
    const ca_id=document.querySelector('script[business_id_app_gpt][id_app_gpt]').getAttribute('id_app_gpt');
    const ca_business_id=document.querySelector('script[business_id_app_gpt][id_app_gpt]').getAttribute('business_id_app_gpt');
    const user_id=document.querySelector('script[user_id_app_gpt][id_app_gpt]').getAttribute('user_id_app_gpt');
    const src=document.querySelector('script[business_id_app_gpt][id_app_gpt][src]').getAttribute('src');
    
    var element_id_script_app_set = document.querySelector("script[element_id_app_gpt]");
    if(element_id_script_app_set){ 
          element_id_app_set = element_id_script_app_set.getAttribute("element_id_app_gpt");
    }
    if(!element_id_app_set){ 
          element_id_app_set = "u9sst8htz0xhrrdsgpt";
    } 
    ...
    async function getStyle() {
        ...
        try {
            ...
            if (txt!==undefined && txt.length > 24) {
                txt = txt.slice(0, 24) + '...';
            }
    ...
    (async () => {
        ...
        // }
        var htmlData=`<link href="${baseUrl}chat/app_gpt_style.css" rel="stylesheet">
    ...
    <section class="chat-user-card">
                ...
                <div class="login-box">
                    <h2>Get Started</h2>
                        ...
                        <div class="text-end submit-btn-side">
                            <button type="submit" class="btn btn overflow-hidden ">
                                <span></span>
                                <span></span>
                                <span></span>
                                <span></span>
        ...
        <div class="form-site mb-5 form-style-1 form-response d-flex align-items-center justify-content-center" id="theme_change_2" style="display:none !important">
            <div>
                <h4>Response</h4>
                <div class="login-box">
                    <div class="content-inner" id="response_contents">
        ...
        questionBox.innerHTML = `<input type="hidden" name="question_id_${index}" value="${question.caq_id}"/> <h3 class="question"> <span class="text-gray">${question.caq_question_label} ${question.caq_question_text} </span></h3>`;
    ...
    case "7": // Radio
        ...
        inputElement += radioOptions.map((option, i) => `
            <div class="radio-wrapper d-flex align-items-center gap-2">
                <input type="radio" placeholder="Enter Your Answer" name="answer_${index}" id="question_option_${index}_${i}" value="${option.trim()}">
   ...
   case "8": // Checkbox
    ...
    inputElement += checkboxOptions.map((option, i) => `
        <div class="checkbox-wrapper-4">
            <input class="inp-cbx" id="checkbox_${index}_${i}" placeholder="Enter Your Answer" name="answer_${index}[]" type="checkbox" value="${option.trim()}"/>
    ...
    case "9": // Select
        ...
        inputElement = `
            <select placeholder="Enter Your Answer" name="answer_${index}" id="">
...
themeChangeIds.forEach(id => {
  ...
  if (themeChangeElement) {
    ...
    function startOver(){
         document.getElementById("theme_change_1").style.setProperty("display", "block", "important");
         document.getElementById("theme_change_2").style.setProperty("display", "none", "important");
    ...
    document.getElementById("gpt_apps_form").addEventListener("submit", function (event) {
...
async function sendFormData(formDatas) {
    ...
    try {
        ...
        const json = await response.json(); 
        document.getElementById("theme_change_1").style.setProperty("display", "none", "important");
        document.getElementById("theme_change_2").style.setProperty("display", "block", "important");
